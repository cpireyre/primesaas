...
/*   Updated: 2019/09/29 10:40:33 by cpireyre         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static uint64_t		ft_get_ran(uint64_t mod)
{
	uint64_t	res;
...
static uint64_t		ft_sq_mod(uint64_t to_sq, uint64_t mod)
{
	uint64_t	res;
...
static uint64_t		ft_rapid_expo_mod(uint64_t nbr, uint64_t exp, uint64_t mod)
{
	if (!(exp))
		return (1);
	if (1 & exp)
